...
- (void)repositionViews
{
  ...
  if (self.accessoryView) {
    ...
    firstLineRightBoundary = CGRectGetMinX(accessoryRect) - TEXT_FIELD_HSPACE;
  }
  ...
  for (UIView *tokenView in self.tokenViews) {
    ...
    if (!self.singleLine && curX + CGRectGetWidth(tokenRect) > tokenBoundary) {
      // Need a new line
      curX = PADDING_LEFT;
    ...
    curX = CGRectGetMaxX(tokenRect) + TEXT_FIELD_HSPACE;
  ...
  if (!self.singleLine && availableWidthForTextField < MINIMUM_TEXTFIELD_WIDTH) {
    isOnFirstLine = NO;
...
#pragma unused(isOnFirstLine)
  ...
  if (self.singleLine) {
    availableWidthForTextField = MINIMUM_TEXTFIELD_WIDTH;
  }
    ...
    if (self.contentSize.width != position) {
      ...
      if (self.contentOffset.x != offset) {
        [UIView animateWithDuration:0.3f animations:^{
          [self setContentOffset:(CGPoint) { offset, 0 } animated:NO];
        }];
      }
    }
  }
